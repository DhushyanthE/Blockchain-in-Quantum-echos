...
export class QuantumSecurityService {
  ...
  private calculateSecurityScore(model: TrainedQuantumModel): number {
    ...
    const metrics = model.metrics || model.finalMetrics;
    ...
    // Make sure we're treating metrics as having the quantumResistance property
    const metricsWithQuantumResistance = metrics as SecurityMetrics;
    
    if (metricsWithQuantumResistance.quantumResistance !== undefined) {
      score += metricsWithQuantumResistance.quantumResistance * quantumResistanceWeight;
  ...
  private identifyVulnerabilities(model: TrainedQuantumModel): string[] {
    ...
    if (metrics) {
      ...
      // Cast metrics to SecurityMetrics to access quantumResistance property
      const metricsWithQuantumResistance = metrics as SecurityMetrics;
      
      if (metricsWithQuantumResistance.quantumResistance && metricsWithQuantumResistance.quantumResistance < 0.7) {
