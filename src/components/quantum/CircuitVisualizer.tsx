...
// Add a type for insights
interface CircuitInsights {
  complexity: number;
  optimization: string[];
...
// Add a type for the simulation result
interface SimulationResult {
  success: boolean;
  executionTime: number;
  processingCapacity: number;
  result: string;
  insights?: CircuitInsights; // Make insights optional
}

// Rename the component to match the import in QuantumCircuits.tsx
export function CircuitVisualizer() {
  ...
  const simulateCircuit = () => {
    ...
    setTimeout(() => {
      // Create a mock simulation result with insights
      const mockResult: SimulationResult = {
        success: true,
        executionTime: Math.random() * 500 + 100,
        processingCapacity: Math.random() * 0.5 + 0.5,
        result: Math.random() > 0.5 ? "0101" : "1010",
        insights: {
          complexity: Math.random() * 0.8 + 0.2,
          optimization: [
            "Gate cancellation possible between H-H pairs",
            "CNOT operation can be optimized further"
          ],
          quantumAdvantage: Math.random() * 0.7 + 0.3,
          reliability: Math.random() * 0.6 + 0.4
        }
      };
  ...
  // Display circuit insights if available
  const renderInsights = () => {
    if (!simulationResult || !simulationResult.insights) {
      return null;
    }
    ...
    return (
      <div className="bg-gray-900/60 p-3 rounded-lg border border-gray-800 mt-4">
        <h3 className="text-sm font-medium text-white mb-2">Circuit Insights</h3>
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div>
            <div className="text-gray-400">Complexity</div>
            <div className="text-white">{(insights.complexity * 100).toFixed(1)}%</div>
          </div>
          <div>
            <div className="text-gray-400">Quantum Advantage</div>
            <div className="text-white">{(insights.quantumAdvantage * 100).toFixed(1)}%</div>
          </div>
        </div>
        {insights.optimization.length > 0 && (
          <div className="mt-2">
            <div className="text-gray-400 mb-1">Optimization Suggestions</div>
            <ul className="list-disc list-inside text-xs text-white">
              {insights.optimization.map((tip, i) => (
...
// Also keep the original component name for backward compatibility
export const QuantumCircuitVisualizer = CircuitVisualizer;
